...
class ExtractImagesFromPdf(Resource):
    def get(self):
        cloudinary_pdf_urls = request.args.getlist('pdf_url')
        if not cloudinary_pdf_urls:
            return {"error": "pdf_url parameter is required"}, 400
        
        all_extracted_image_links = []
        for cloudinary_pdf_url in cloudinary_pdf_urls:
            extracted_image_links = extract_images_from_pdf(cloudinary_pdf_url)
            all_extracted_image_links.extend(extracted_image_links)
        delete_files_with_extension("./assets")
        
        return {"image_links": all_extracted_image_links}, 200
...
class UploadPdf(Resource):
    def post(self):
        # Parse the incoming request for 'pdf' files
        parse = reqparse.RequestParser()
        parse.add_argument('pdf', type=werkzeug.datastructures.FileStorage, location='files', action='append')
        args = parse.parse_args()
        ...
        if pdf_files:
            ...
            for pdf_file in pdf_files:
                # Upload the file to Cloudinary
                upload_response = cloudinary.uploader.upload(pdf_file, resource_type = "raw")
                # Append the URL of the uploaded file to our list
                pdf_urls.append(upload_response['url'])
            
            # Return the URLs of the uploaded files
            return {"pdf_urls": pdf_urls}, 200
        else:
            return {"error": "No files were provided"}, 400
