...
class _FilePickerDemoState extends State<FilePickerDemo> {
  ...
  List<Map<String, dynamic>> allImageLinks = [];
  ...
  Future<void> extractImages(List<String> pdfUrls) async {
    ...
    List<Map<String, dynamic>> allImageLinks =
        []; // List to hold all the image links
      ...
      if (response.statusCode == 200) {
        ...
        for (var imageUrl in imageLinks) {
          ...
          allImageLinks.add({
            'url': imageUrl,
            'size':
                ((int.parse(imageResponse.headers['content-length'] ?? '0')) /
                        1024)
                    .toStringAsFixed(2), // Size in KB
            'format': (imageResponse.headers['content-type']?.split('/') ??
                ['', ''])[1], // Format as extension
          });
        }
        print(allImageLinks);
      } else {
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // ),
            SvgPicture.asset(
              'assets/pdficon.svg', // Replace with the path to your SVG file
